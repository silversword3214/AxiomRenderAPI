...
public class RenderCore {
    ...
    /**
     * Täytetty pyöristetty suorakulmio.
     */
    ...
    /**
     * Pyöristetty suorakulmio, jossa voi valita pyöristettävät kulmat.
     */
    // RenderCore.java, korjattu addRoundedRectCustom
    ...
    public void addRoundedRectCustom(float x, float y, float w, float h, float radius, int color,
        ...
        if (!topLeft) {
            // Vasen yläkulma: neliö koko radius x radius
    ...
    /**
     * Pyöristetyn suorakulmion ääriviiva.
     */
    public void addRoundedRectOutline(float x, float y, float w, float h, float radius, float thickness, int color) {
        ...
        // Kaaret (käytetään paksuja viivoja)
        // Vasen yläkulma
        float cx1 = x + radius;
        float cy1 = y + radius;
        for (int i = 0; i < segments; i++) {
        ...
        // Suorat osat
        addLineThick(x + radius, y, x + w - radius, y, thickness, color);               // yläreuna
        addLineThick(x + w, y + radius, x + w, y + h - radius, thickness, color);       // oikea reuna
        addLineThick(x + w - radius, y + h, x + radius, y + h, thickness, color);       // alareuna
        addLineThick(x, y + h - radius, x, y + radius, thickness, color);               // vasen reuna
    }
    ...
    public void addTexturePart(Identifier texture, float x, float y, float width, float height,
        ...
        if (uiTexturedPipeline == null) return;
        Batch batch = batches.computeIfAbsent(uiTexturedPipeline,
